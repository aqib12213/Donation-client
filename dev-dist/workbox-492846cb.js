					...
					if (matchResult instanceof Promise) {
						logger.warn(
							`While routing ${getFriendlyURL(url)}, an async ` +
								"matchCallback function was used. Please convert the following route to use a synchronous matchCallback function:",
							route
						);
					}
					...
					if (responseToCache.status === 0) {
						logger.warn(
							`The response for '${this.request.url}' ` +
								"is an opaque response. The caching strategy that you're using will not cache opaque responses by default."
						);
					} else {
						logger.debug(
							`The response for '${this.request.url}' ` +
    ...
    */
	...
	class NavigationRoute extends Route {
		...
		_match({ url, request }) {
			...
			logger.log(
				`The navigation route ${pathnameAndSearch} is not ` +
					"being used, since the URL being navigated to doesn't match the allowlist."
			);
			return false;
		}
	}
...
//# sourceMappingURL=workbox-492846cb.js.map
